...
//Esp8266做服务端
//只对 ID为0的连接发送消息
// 如果想要对其他连接发消息，想要修改Multiple_ID_0的値
	...
	while(1)
			...
			//printf("Data_RX_BUF = %s\n\n", ESP8266_Fram_Record_Struct .Data_RX_BUF);
			 if(CONNECT_ID())
				{
				ESP8266_Fram_Record_Struct .InfBit .FramLength = 0;
				Server_8266_AP_send("..................请输入 WiFi 账户和密码	SSID:#####, PWD:#######\r\n");
				Flag_wifi = 0;
				break;
				}
			 else
...
void Rec_IP(void)
{
	char* str;
	u8 i = 0 ;
	Server_8266_AP_send("请输入	pc的IP号\n");
	ESP8266_Fram_Record_Struct .InfBit .FramLength = 0;
	Flag_ip = 0;
	while(1)
		{
